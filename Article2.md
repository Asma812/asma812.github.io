...
*   **1\. Introduction:** Overview of the 5G security trust model and its reliance on public-key infrastructure (PKI).
    
*   **2\. Vulnerability Mapping in the 5G Core:**
    
    *   **Signaling (HTTP/2 & TLS):** Risks to the communication between Network Functions (AMF, SMF, UDM).
        
    *   **Authentication & Key Agreement (5G-AKA):** How an attacker could spoof a home network or intercept SUCI (Subscription Concealed Identifier).
        
    *   **Key Exchange:** Vulnerability of Diffie-Hellman/Elliptic Curve exchanges used for session keys.
        
*   **3\. Quantum Attack Vectors:**
    
    *   **Shor's Algorithm:** Targeting RSA-2048 and ECC.
        
    *   **Grover's Algorithm:** The 50% reduction in symmetric key strength (AES-128 becoming equivalent to 64-bit).
        
*   **4\. Transitioning to Post-Quantum 5G:**
    
    *   Implementation of **ML-KEM** (Kyber) and **ML-DSA** (Dilithium) in 3GPP standards.
        
    *   Sidecar proxy architectures for PQC-enabled 5G.
        
*   **Conclusion:** The necessity of updating the 5G security roadmap to include quantum-resistant signaling.
...
*   **The PKI Dependency**: These TLS connections rely on a Public Key Infrastructure (PKI) to authenticate the NFs and encrypt the signaling data.
...
### **3.3 The Cumulative Effect on 5G Trust**
